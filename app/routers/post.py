...
# NEVER f STRINGS INSTEAD USE %s AND A TUPLE
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), 
    ...
    # unpack the post pydantic object into the SQLAlchemy model
    db.add(new_post)
    db.commit() # This saves the changes to the database
    db.refresh(new_post) # This will get the new post from the database
    return new_post
...
def get_posts(db: Session = Depends(get_db), 
    ...
    posts = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join( #  type: ignore
...
def get_post(id: int, db: Session = Depends(get_db),
             current_user: int = Depends(oauth2.get_current_user)):
    # you have to convert id to str because the data fetched from the database is in str format
    # alwways put in a comma after a single value tuple
